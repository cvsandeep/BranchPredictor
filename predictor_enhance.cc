...
inline void bias_predictor(void){
  for(int i=0; i<1024; i++){
      // Biasing Local Predictor for weak not taken
      LocalPredictor[i][0]=1; LocalPredictor[i][1]=1;
  }
  for(int i=0; i<4096; i++){
      GlobalPredictor[i][0]=1;
  }
}
/**********************************************************************
 *******************LOCAL PREDICTOR ***********************************
...
inline bool get_global_prediction(unsigned int pc_index) {
	unsigned int x = pc_index ^ PathHistory.to_ulong();
	return GlobalPredictor[x][1];
}

inline void update_global_predictor(unsigned int pc_index, bool taken) {
	unsigned int x = pc_index ^ PathHistory.to_ulong();
...
inline void update_choice_predictor(unsigned short index, bool outcome) {
        ...
        bool globalChoice = get_global_prediction(index);
            ...
            if (!br->is_conditional) {
            	...
            	if (br->is_call) {   // Call push return address in stack
					...
					if (!get_choice_prediction()) {
							...
							//Use Global Table
							prediction =  get_global_prediction(pc_index);
					}
            	}
            }
    ...
    void PREDICTOR::update_predictor(const branch_record_c* br, const op_state_c* os, bool taken)
        {
			...
			update_global_predictor(pc_index, taken);
